...
def get_impact_figure(df):
    ...
    # We iterate and append #2, #3 etc. if needed
    final_labels = []
    seen_labels = {}
    
    # Iterate in the order they will be plotted (which is sorted by Deaths ascending)
    # But for naming consistency, maybe we want to process them? 
    # Actually, just ensuring uniqueness is enough for Plotly to separate them.
...
AGENT_DEFINITIONS = {
    'Pyroclastic Flow': {
        'image': 'pyroclastic_flow.webp',
        'definition': 'A chaotic, super-heated mixture of gas, ash, and rock that flows down the side of a volcano at incredibly high speeds (often >80 km/h).',
        'danger': 'It is the deadliest volcanic hazard. It destroys everything in its path through impact and extreme heat (up to 1,000°C). You cannot outrun it.'
    },
    'Mudflow (Lahar)': {
        'image': 'lahar.jpg',
        'definition': 'A slurry of volcanic ash, rock debris, and water that flows like wet concrete. Can be Primary (eruption-triggered) or Secondary (rain-triggered).',
        'danger': 'They can travel tens of kilometers away from the volcano, burying valleys and towns in thick mud that hardens like cement.'
    },
    'Tephra (Ash)': {
        'image': 'Tephra.webp',
        'definition': 'Fragments of rock and glass ejected into the air. "Ash" refers to the smallest particles (<2mm), while "Tephra" is the general term.',
        'danger': 'Ash can collapse roofs due to its weight, destroy jet engines in flight, and cause respiratory problems. It is the most widespread hazard.'
    },
    'Waves (Tsunami)': {
        'image': 'tsunami.jpg',
        'definition': 'Giant ocean waves triggered by a volcanic event, such as a caldera collapse, a debris avalanche entering the sea, or an underwater explosion.',
        'danger': 'These waves can devastate coastlines far from the volcano itself (e.g., the 1883 Krakatoa tsunami).'
    },
    'Lava Flow': {
        'image': 'lava_flow.jpg',
        'definition': 'Streams of molten rock pouring from a vent.',
        'danger': 'While visually terrifying, lava usually moves slowly enough for people to walk away. The main risk is the total destruction of property and infrastructure.'
    },
    'Gas': {
        'image': 'gaz.jpg',
        'definition': 'Volcanic gases (water vapor, carbon dioxide, sulfur dioxide, etc.) released from magma.',
        'danger': 'Gases like Sulfur Dioxide can cause acid rain. Carbon Dioxide is heavier than air and can settle in low-lying areas, silently suffocating people and animals.'
    },
    'Indirect': {
        'image': 'indirect.webp',
        'definition': 'Deaths or damage not caused by the physical eruption itself but by its aftermath.',
        'danger': 'This historically accounts for massive death tolls. It includes starvation (crops killed by ash), disease (water contamination), and climate cooling.'
    },
    'Avalanche': {
        'image': 'Avalanche.jpg',
        'definition': 'The sudden collapse of a volcano\'s unstable flank (side), causing a massive landslide of rock and soil.',
        'danger': 'This is often the trigger for explosive eruptions (like Mt. St. Helens). It can bury vast areas and trigger tsunamis if it hits water.'
    },
    'Floods': {
        'image': 'floods.jpg',
        'definition': 'Water floods caused by the eruption disrupting natural water systems or melting snow/ice without creating a full lahar.',
        'danger': 'It extends the damage zone beyond the volcano, often affecting people downstream who didn\'t expect to be at risk.'
    },
    'Seismic': {
        'image': 'seismic.jpg',
        'definition': 'Earthquakes generated by magma moving underground or by the structural failure of the volcano.',
        'danger': 'While usually smaller than tectonic quakes, they can damage buildings near the volcano and trigger landslides or tsunamis.'
    },
    'Electrical': {
        'image': 'electrical.avif',
        'definition': 'Lightning storms generated within the volcanic plume itself.',
        'danger': 'The intense static electricity is created by ash particles rubbing together. It poses a risk to people near the eruption and aviation.'
    },
    'Unknown': {
        'image': None,
        'definition': 'Cause of death or damage not specified in the historical record.',
        'danger': 'N/A'
    },
    'Tsunami': {
        'image': 'tsunami.jpg',
...
app.layout = html.Div(
    children=[
        ...
        html.Div(
            children=[
                html.Label("Volcano Types", style={'color': 'white', 'marginBottom': '10px', 'display': 'block'}),
                dcc.Checklist(
                    id='volcano-type-checklist',
                    options=[{'label': t, 'value': t} for t in sorted(df['Type'].unique())],
                    value=sorted(df['Type'].unique()),
                    inline=True,
                    style={'color': 'white'},
                    inputStyle={'marginRight': '5px', 'marginLeft': '15px'}
                )
            ],
            style={"marginBottom": "30px", "padding": "20px", "backgroundColor": "#1e1e1e", "borderRadius": "10px"}
        ),
...
@app.callback(
    Output('agent-insight-box', 'children'),
    [Input('with_and_without_indirect_deaths_by_type', 'hoverData'),
     Input('vei-deaths-graph', 'hoverData')]
)
def update_agent_insight(bar_hover, heatmap_hover):
    ctx_msg = ctx.triggered_id
    
    agent_name = None
    
    if ctx_msg == 'with_and_without_indirect_deaths_by_type' and bar_hover:
        # Try to get agent name from bar chart
        # x value is the Hazard name
        try:
            agent_name = bar_hover['points'][0]['x']
        except:
            pass
            
    elif ctx_msg == 'vei-deaths-graph' and heatmap_hover:
        # Try to get agent name from heatmap
        # x value is the Hazard Agent
        try:
            agent_name = heatmap_hover['points'][0]['x']
        except:
            pass
    ...
    # Normalize name if needed (e.g. remove extra text)
    # The keys in AGENT_DEFINITIONS match the labels used in charts (hopefully)
    
    info = AGENT_DEFINITIONS.get(agent_name)
    
    if not info:
        # Fallback for partial matches or codes
        for key in AGENT_DEFINITIONS:
            if key in agent_name:
                info = AGENT_DEFINITIONS[key]
                break
    
    if not info:
        return html.Div([
            html.H4("Agent Definition", style={'color': '#ff5722', 'marginBottom': '5px'}),
            html.P("Hover over agents to see their definition", style={'color': '#888', 'fontStyle': 'italic', 'fontSize': '12px', 'marginBottom': '15px'}),
            html.H4(agent_name, style={'color': '#ff5722'}),
            html.P("No detailed definition available.", style={'color': '#ccc'})
        ])
    ...
    content = [
        html.H4("Agent Definition", style={'color': '#ff5722', 'marginBottom': '5px'}),
        html.P("Hover over agents to see their definition", style={'color': '#888', 'fontStyle': 'italic', 'fontSize': '12px', 'marginBottom': '15px'}),
        html.H4(agent_name, style={'color': '#ff5722', 'marginBottom': '10px'}),
    ]
    
    if info['image']:
        content.append(html.Img(src=app.get_asset_url(info['image']), style={'width': '100%', 'borderRadius': '5px', 'marginBottom': '10px'}))
        
    content.append(html.P([html.B("Definition: "), info['definition']], style={'fontSize': '14px', 'marginBottom': '10px'}))
    content.append(html.P([html.B("Why it's dangerous: "), info['danger']], style={'fontSize': '14px', 'color': '#ccc'}))
...
def update_dashboard(selected_country, year_range, selected_vei_metric, selected_secondary_metric, selected_heatmap_metric, map_mode, time_mode, type_scale, type_metric, map_selection, selected_volcano_types, selected_region):
    ...
    if selected_volcano_types is not None:
        dff = dff[dff['Type'].isin(selected_volcano_types)]
    ...
    # Regional
    fig_regions = render_top10_deadliest_regions(dff)
    fig_sunburst = render_region_volcano_sunburst(dff)
    fig_treemap = render_deadliest_volcanoes_treemap(dff)
    fig_donut = render_deadliest_volcanoes_donut(dff)
    
    # Injuries
    fig_inj_scatter = render_deaths_vs_injuries_scatter(dff)
    fig_inj_bubble = render_deaths_vs_injuries_bubble(dff)
    fig_inj_ratio = render_top10_injury_ratio(dff)
    fig_inj_ratio_sc = render_injury_ratio_scatter(dff)
    
    # Distributions
    fig_boxplot = render_death_boxplot(dff)
    fig_boxplot_type = render_death_boxplot_by_type(dff)
    fig_pareto = render_pareto_chart(dff)
    fig_loglog = render_loglog_heavytail(dff)
    fig_corr = render_correlation_heatmap(dff)
